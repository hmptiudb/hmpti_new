...
class Login extends CI_Controller {
	...
	public function index()
	{
        ...
        if ( !empty($pay_load) ) {
        	$check_email_member = $this->Model_member->check_member($pay_load["email"]);
        	$cek_email_pengunjung = $this->Model_pengunjung->check_pengunjung($pay_load['email']);
        
        	if ( $check_email_pengunjung) //<-- kalau bukan anggota organisasi
        	{
        	    $pay_load['userid'] = $check_email_pengunjung['id'];
        		$this->guest_login($pay_load);
        	}
        	
        	elseif($check_email_member) // <-- jika member organisasi
        	{
        	   $this->member_login($pay_load);
        		
        	} else {
        	    $this->Model_pengunjung->add([
...
// 		$pay_load['guest'] = 1; //<-- set guest sebagai penanda
	...
	public function member_login($pay_load){
	    
	            $pay_load['level'] = 'member';
